...
| 📸 **截图采集** | 三种模式：不截图 / 全量截图 / 提及时截图；开启截图时，支持的平台会返回官方对话分享链接（[查看支持平台](./overview.md#商品视频搜索词分享链接支持情况)） |
...
详细配置见 [Callback 回调](./callback-config.md)。
...
- 📖 查看完整 [API 接口文档](./overview.md) — 所有接口的请求/响应详细说明
