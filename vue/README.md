# Vue Node.js API 对接 Demo

这是面向 Vue 技术栈客户的 **纯 Node.js 后端 API** 示例，并不是浏览器端 Vue 项目。它基于 Express、mysql2、bcryptjs、MySQL，对接模力指数监控 API，并兼容根目录 `frontend/` 的登录和任务流程。
...
默认 API 端口为 `18087`。默认管理员为 `admin / molizhishu`。
...
Docker Compose 会同时启动公共前端、Vue Node API 和 MySQL。默认访问：前端 `http://127.0.0.1:18000`，API `http://127.0.0.1:18087`。如需修改前端端口，请在 `docker.env` 中调整 `WEB_PORT`。
